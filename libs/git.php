...
#						if (file_exists($fullpath . $repoSuffix . $headFile) && static::getOwner($fullpath, $repoSuffix) !== null) {
		...
		static::$repos = array_combine($valid, $repos);
		sort($repos);
	...
	public function parse($proj, $what) {
		$gitBinary = System::get('git_binary');

		$cmd1 = "GIT_DIR=" . static::$repos[$proj] . System::get('repo_suffix') . " {$gitBinary} rev-parse  --symbolic --" . escapeshellarg($what) . "  2>&1";
		$out1 = array();
		$bran = array();
		exec($cmd1, $out1);
		for($i = 0; $i < count($out1); $i++) {
			$cmd2="GIT_DIR=" . static::$repos[$proj] . System::get('repo_suffix') . " {$gitBinary} rev-list --max-count=1 " . escapeshellarg($out1[$i]) . " 2>&1";
			$out2 = array();
			exec($cmd2, $out2);
			$bran[$out1[$i]] = $out2[0];
		}
	...
	public static function shortlogs($proj) {
		return static::getLastNCommits($proj);
	}
	...
	public static function commit($proj, $commit) {
		...
		return static::getLastNCommits($proj, $options);
	}

	public static function blob($proj, $path) {
		if (is_file(static::$repos[$proj] . DS . $path)) {
			$File = new File(static::$repos[$proj] . DS .$path);
			return array(
				'content' => $File->read(),
				'ext' => $File->ext()
			);
...
echo $proj . "<br>\n";
	...
	public static function diff($proj, $commit) {
		...
		$out = array();
		$cmd = "GIT_DIR=" . static::$repos[$proj] . System::get('repo_suffix') . " {$gitBinary} show {$commit} --format=\"%b\" 2>&1";
		exec($cmd, $out);
	...
	private static function getLastNCommits($proj, $options = array()) {
		...
		$cmd = "GIT_DIR=" . static::$repos[$proj] . System::get('repo_suffix') . " {$gitBinary} rev-list {$query} {$params} --pretty=format:\"{$format}\"";
